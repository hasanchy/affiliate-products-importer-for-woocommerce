...
class AsinVerification extends Endpoint {
	...
	public function asin_verification( WP_REST_Request $request ) {
        ...
        if($asin_codes == "" ){
            return new WP_Error('rest_azoncom_amazon_product_fetch', 'ASIN Codes parameter value can not be empty', ['status' => 500]);
        }
		...
		if ( ! empty( $access_key ) && ! empty( $secret_key ) && ! empty( $country_code ) && ! empty( $affiliate_id ) ) {
			...
			try {
                ...
                $items = (isset($result->ItemsResult->Items)) ? $result->ItemsResult->Items : [];
                $fetch_errors = (isset($result->Errors)) ? $result->Errors : [];
