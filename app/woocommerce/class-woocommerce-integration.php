...
class WooCommerceIntegration extends Base {
	...
	public function init() {
        \add_filter( 'woocommerce_get_price_html', [$this, 'amz_disclaimer_price_html'], 100, 2 );

		\add_filter( 'wp_get_attachment_image_src', [$this, 'azoncom_replace_attachment_image_src'], 100, 2 );
		...
		if (is_numeric($attachment_id) && $attachment_id > 0) {
			...
			if (isset($image_data['img_url']) && $image_data['img_url'] != '') {
				...
				if ($image_url) {
					...
					return [$image_url, 800, 600, false];
				}
			}
		}
